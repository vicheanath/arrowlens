...
impl DatasetLoader for JsonLoader {
    async fn load_preview(&self, path: &str, limit: usize) -> Result<LoaderPreview> {
        ...
        ctx.register_json("_preview", path, NdJsonReadOptions::default())
            .await?;
    ...
    async fn infer_schema(&self, path: &str) -> Result<String> {
        let ctx = SessionContext::new();
        ctx.register_json("_schema", path, NdJsonReadOptions::default())
            .await?;
    ...
    async fn count_rows(&self, path: &str) -> Result<u64> {
        let ctx = SessionContext::new();
        ctx.register_json("_count", path, NdJsonReadOptions::default())
            .await?;
